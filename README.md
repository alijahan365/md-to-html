...
| :--- | :--- | :--- |
| **Ali Jahan Riashad** *(Lead)* | `242-15-846` | • Semantic Analyzer (`semantic.h`, `semantic.c`)<br>• Web Studio SPA (`index.html`, `style.css`, `compiler.js`)<br>• CI/CD Workflow (`.github/workflows/ci.yml`)<br>• Comprehensive Documentation (`README.md`, `LICENSE`) |
| **Araft Islam** *(Contributor)* | `242-15-388` | • AST Structs & Allocation (`utils.h`, `utils.c`)<br>• Lexical Scanner (`lexer.h`, `lexer.c`, `lexer.l`)<br>• Syntax Parser (`parser.h`, `parser.c`, `parser.y`)<br>• Symbol Table (`symbol_table.h`, `symbol_table.c`)<br>• HTML Code Generator (`html_generator.h`, `html_generator.c`)<br>• CLI Driver & Makefile (`main.c`, `Makefile`, `sample.md`) |
